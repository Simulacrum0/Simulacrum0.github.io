...
class PaintableCanvas
{
	...
	getMousePos( e )
	{
		...
		const [ dx, dy ] = [
			( width ? this.width / parseInt( width ) : 1.0 ), ( height ? this.height / parseInt( height ) : 1.0 ),
		];
...
class Pass
{
	constructor( w, quad, materialProperties )
	{
		...
		this.program = w.createProgram(
			this.vertexShader, `${prefix}${this.fragmentShader}`
		);
...
class WebGL2MicroLayer
{
	...
	createRenderTarget( width, height, overrides = {}, name = undefined )
	{
		...
		this.framebuffers.set( renderTargetName,
		{
			framebuffer,
			texture,
			width,
	...
	createFullscreenQuad()
	{
		...
		this.gl.bufferData(
			this.gl.ARRAY_BUFFER, new Float32Array( [ -1, -1, 1, -1, -1, 1, 1, 1 ] ), this.gl.STATIC_DRAW
		);
		return {
			buffer,
			size: 2
...
function webGlInit(
	context, width, height, materialProperties, renderTargetOverrides = {}, extra = {}
)
{
	const
	{
...
class BaseSurface
{
	...
	{
		this.surface = new PaintableCanvas(
		{
			width,
			height,
			radius
		...
		return intializeCanvas(
		{
			...
			} ) =>
			{
				...
				this.drawUniforms.color = [
					this.surface.currentColor.r / 255.0, this.surface.currentColor.g / 255.0, this.surface.currentColor.b / 255.0, alpha,
				];
			},
	...
	innerInitialize()
	{
		...
		this.pixelsBetweenProbes = addSlider(
		{
			...
			options:
			{
				min: 0,
				max: 4,
				value: this.rawBasePixelsBetweenProbesExponent,
				step: 1
			},
		...
		this.firstLayerSlider = addSlider(
		{
			...
			{
				min: 1,
				max: this.radianceCascades,
				value: this.radianceCascades,
				step: 1
			},
		} );
		...
		this.radiusSlider = addSlider(
		{
			...
			options:
			{
				min: urlParams.get( "rcScale" ) ?? 1.0,
				max: 100.0,
				step: 0.1,
				value: this.surface.RADIUS
			},
		} );

const BriDzSa__Da_vsg = "PUB_v0.114"; 
...
async function Hre2_Ke__BriYa()
{
	...
	SaNo__TaKeDy_v =
	[
		  { Va: "Guest", Gwz: "👤", Che: false }
		, { Va: "Ghost", Gwz: "👻", Che: false }
		, { Va: "Kiosk", Gwz: "🎰", Che: false }
		, { Va: "Pro", Gwz: "🧑‍💻", Che: true }
	];
...
function KeMeTrx()
{
	...
	if( !MxPo_Trx_l )return;

	MxPo_Trx_l.width = window.devicePixelRatio * MxPo_Trx_l.clientWidth;
	MxPo_Trx_l.height = window.devicePixelRatio * MxPo_Trx_l.clientHeight;
	...
	// Draw horizontal lines
	for (let y = 0; y <= MxPo_Trx_l.height; y += TiGy_wf )
	{
		Sx_l.beginPath();
		Sx_l.moveTo(0, y);
		Sx_l.lineTo(MxPo_Trx_l.width, y);
		Sx_l.stroke();
	}

	// OUTER GRID
	Sx_l.strokeStyle = "#DDDDDD";
	Sx_l.lineWidth = 2;
...
async function KoDz__Ya( )
{
	...
	[
		...
		, Hrz7_Kru__ChaSySmz( "Hry5_Smz", "WzMx", "Do", "WG", SySmz__Kri_yk, { KaVy: "0" } )
	...
	// DoXR
	//Hrz7_Kru__ChaSySmz( "Hri2_Ke", "BzMe", "Do", "XR", "Hri2_Ke", SySmz__HoKri_yk, { SmzYz: "0" } );
	// DoGLES
